...
app.post("/api/allRemote", async (req, res) => {
  ...
  try {
    ...
    for (const movie of remoteMovies) {
      ...
      let doodliData, streamTapeData, videHideData, streamWishData, veevData, youtubeData;
      ...
      try {
        ...
        responses.push({ service: "Vidhide", result: videHideData, message: videHideData.msg });
      } catch (error) {
      ...
      // StreamWish upload
      try {
        streamWishData = await remoteStreamwish(movie?.url, accountData.streamwishKey);
        responses.push({ service: "StreamWish", result: streamWishData, message: streamWishData.msg });
      } catch (error) {
        console.error("Error uploading to StreamWish:", error.message);
      }
      ...
      try {
        ...
        responses.push({ service: "veevData", result: veevData, message: veevData.msg });
      } catch (error) {
      ...
      if (youtubeData) {
        ...
        formData.append("download_link2", `https://dood.li/d/${doodliData?.result?.filecode}`);
        formData.append("iframe_link2", `https://dood.li/d/${doodliData?.result?.filecode}`);
        formData.append("download_link3", streamTapeData?.result?.id);
        formData.append("iframe_link3", streamTapeData?.result?.id);
        formData.append("download_link4", `https://vidhidehub.com/file/${videHideData?.result?.filecode}`);
        formData.append("iframe_link4", `https://vidhidehub.com/embed/${videHideData.result.filecode}`);
        formData.append("download_link5", `https://hlswish.com/${streamWishData?.result?.filecode}`);
        formData.append("iframe_link5", `https://hlswish.com/e/${streamWishData.result.filecode}`);
        formData.append("download_link6", `https://veev.to/d/${veevData?.result?.filecode}`);
        formData.append("iframe_link6", `https://veev.to/e/${veevData?.result?.filecode}`);
